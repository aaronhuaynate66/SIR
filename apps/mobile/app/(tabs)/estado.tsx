...
function hapticLight() {
  try {
    ...
    const H = require('expo-haptics') as { impactAsync: (s: unknown) => Promise<void>; ImpactFeedbackStyle: { Light: unknown } };
...
function hapticSuccess() {
  try {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const H = require('expo-haptics') as { notificationAsync: (t: unknown) => Promise<void>; NotificationFeedbackType: { Success: unknown } };
...
export default function EstadoScreen() {
  ...
  async function handleSave() {
    if (!mood || !energy) { setError('Selecciona tu ánimo y nivel de energía.'); return; }
    setSaving(true);
    setError(null);
    try {
  ...
  function handleReset() {
    setMood(null); setEnergy(null);
    setPhysical([]); setEmotional([]);
    setNotes(''); setResult(null); setError(null);
  }
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.moodRow}>
        {MOODS.map((emoji, i) => (
          <TouchableOpacity
            ...
            onPress={() => { hapticLight(); setMood(i + 1); }}
          >
      ...
      <View style={styles.energyRow}>
        {Array.from({ length: 10 }, (_, i) => i + 1).map(n => (
          <TouchableOpacity
            ...
            onPress={() => { hapticLight(); setEnergy(n); }}
          >
      ...
      <View style={styles.tagRow}>
        {PHYSICAL_TAGS.map(tag => (
          ...
          >
            <Text style={[styles.tagText, physical.includes(tag) && styles.tagTextSelected]}>{tag}</Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <View style={styles.tagRow}>
        {EMOTIONAL_TAGS.map(tag => (
          ...
          >
            <Text style={[styles.tagText, emotional.includes(tag) && styles.tagTextSelected]}>{tag}</Text>
          </TouchableOpacity>
        ))}
      </View>
