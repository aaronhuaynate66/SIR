...
async function getData() {
  ...
  const topUsers = [...byCost.entries()].sort((a, b) => b[1] - a[1]).slice(0, 10);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 28 }}>
        <PeriodCard label="Hoy" cost={d.today.cost} tokens={d.today.tokens} calls={d.today.calls} color="#6366f1" />
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f9fafb' }}>
                {['#', 'Usuario', 'Costo', 'Barra (límite $8)'].map(h => (
